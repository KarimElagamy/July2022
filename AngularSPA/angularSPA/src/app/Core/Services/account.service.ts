...
export class AccountService {
  ...
  Login(loginData:Login):Observable<boolean>{
    return this.httpClient.post<boolean>("https://localhost:7079/api/Account/login", loginData).pipe(map((response:any) => {
      if (response){
  ...
  Register(registerData:Register):Observable<boolean>{
    return this.httpClient.post<boolean>("https://localhost:7079/api/Account/register", registerData);
  }
